...
export default function PersonModal({ onClose, companyId, templates, companyName, companyLogoUrl, companyAddress, customFieldDefs, person }: PersonModalProps) {
    ...
    const previewData: CardData = {
        ...
        full_name_with_titles: (() => { const base = [academicPrefix, previewFirstName, previewLastName].filter(Boolean).join(" "); return academicSuffix ? `${base}, ${academicSuffix}` : base; })(),
        name_with_suffix: academicSuffix ? `${previewFirstName} ${previewLastName}, ${academicSuffix}` : `${previewFirstName} ${previewLastName}`,
